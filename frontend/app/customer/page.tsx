...
export default function CustomerDashboard() {
  ...
  const recentScans = [
    {
      id: 1,
      name: "Amoxicillin 500mg Batch #X-102",
      status: "Authentic",
      date: "2 hours ago",
    },
      ...
      <div className="flex flex-col md:flex-row gap-6">
        <Card className="flex-1 p-6 lg:p-8 border-none bg-gradient-to-br from-primary/10 via-background to-primary/5 shadow-2xl shadow-primary/5 rounded-[2.5rem] relative overflow-hidden group">
          ...
          <div className="relative z-10">
            <Badge className="mb-4 bg-primary/20 text-primary border-none px-3 py-1 rounded-full text-[10px] font-bold tracking-tight">
              Next Dose Reminder
            </Badge>
            ...
            <div className="flex flex-wrap gap-3">
              <Button
                className="rounded-2xl h-12 px-8 font-bold shadow-lg shadow-primary/20 transition-all hover:-translate-y-0.5"
                size="lg"
              >
              ...
              <Button
                variant="outline"
                className="rounded-2xl h-12 px-6 font-bold bg-background/50 backdrop-blur"
                size="lg"
                asChild
              >
        ...
        <Card className="w-full md:w-[320px] p-6 lg:p-8 flex flex-col justify-between border-primary/10 bg-card/50 backdrop-blur-sm rounded-[2.5rem]">
          <div>
            ...
            <p className="text-sm text-muted-foreground leading-relaxed">
              All{" "}
              <span className="text-foreground font-bold">12 medicines</span> in
              your My Medicines have been verified via blockchain.
            </p>
          </div>
          <Button
            variant="link"
            className="h-auto p-0 justify-start mt-6 text-primary font-bold text-xs group"
            asChild
          >
      ...
      <div className="grid lg:grid-cols-3 gap-6 lg:gap-10">
        ...
        <section className="lg:col-span-2 space-y-6">
          <div className="flex items-center justify-between">
            ...
            <Badge
              variant="outline"
              className="rounded-full border-primary/20 text-primary py-0.5"
            >
              2 New Alerts
            </Badge>
          </div>
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground leading-relaxed mb-4">
                    Vitamin D3 supply will run out in 3 days. Order now to avoid
                    disruption.
                  </p>
                  <Button
                    size="sm"
                    variant="outline"
                    className="rounded-xl h-8 font-bold tracking-tight border-amber-500/30 bg-background/50"
                  >
                    Order Now
                  </Button>
                </div>
            ...
            <Card className="p-5 border-primary/20 bg-primary/5 rounded-3xl group hover:bg-primary/10 transition-colors cursor-default">
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground leading-relaxed mb-4">
                    Your medication review with Dr. Sarah is tomorrow at 10:00
                    AM.
                  </p>
                  <Button
                    size="sm"
                    variant="outline"
                    className="rounded-xl h-8 font-bold tracking-tight border-primary/30 bg-background/50"
                  >
                    Add to Calendar
                  </Button>
                </div>
          ...
          <div className="pt-4">
            <Card className="p-6 lg:p-8 bg-muted/30 border-dashed border-2 border-border/50 rounded-[2.5rem] flex flex-col md:flex-row items-center justify-between gap-6">
              <div className="flex items-center gap-6 text-center md:text-left">
                <div className="h-16 w-16 rounded-full bg-background border-4 border-primary flex items-center justify-center text-xl shadow-inner font-bold">
                  85%
                </div>
                <div>
                  <h4 className="font-black text-lg">Adherence Rate</h4>
                  <p className="text-sm text-muted-foreground italic">
                    You're doing better than 70% of members!
                  </p>
                </div>
              </div>
              <Button
                variant="outline"
                className="rounded-2xl h-12 px-6 font-bold bg-background shadow-sm border-primary/20"
                asChild
              >
                <Link href="/customer/cabinet/history">
                  <TrendingUp className="mr-2 h-4 w-4 text-primary" /> View
                  detailed trends
                </Link>
              </Button>
            </Card>
        ...
        <section className="space-y-6">
          ...
          <div className="space-y-3">
            {recentScans.map((scan) => (
              <Card
                key={scan.id}
                className="p-4 rounded-2xl border-border/50 bg-card/30 hover:bg-muted/50 transition-colors cursor-default"
              >
                <div className="flex justify-between items-start mb-2">
                  <h5 className="text-xs font-bold truncate max-w-[150px]">
                    {scan.name}
                  </h5>
                  <Badge className="bg-green-500/10 text-green-500 border-none text-[8px] px-1.5 py-0 font-bold">
                    Authentic
                  </Badge>
                </div>
                <div className="flex items-center justify-between text-[10px] text-muted-foreground font-bold tracking-tighter">
                  <span className="flex items-center gap-1">
                    <Smartphone className="h-3 w-3" /> Mobile Scan
                  </span>
                  <span>{scan.date}</span>
                </div>
              </Card>
            ...
            <Button
              variant="ghost"
              className="w-full rounded-2xl text-xs font-bold text-muted-foreground hover:text-primary transition-colors py-6 border border-dashed border-border group"
              asChild
            >
              <Link href="/verify">
