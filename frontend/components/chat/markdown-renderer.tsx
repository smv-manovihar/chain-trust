...
const BASE_COMPONENTS = {
  h1: ({ children }: React.ComponentPropsWithoutRef<"h1">) => (
    <h1 className="text-3xl font-extrabold tracking-tight mt-8 mb-4 border-b pb-2 first:mt-0">
      {children}
    </h1>
  ),
  ...
  p: ({ children }: React.ComponentPropsWithoutRef<"div">) => (
    <div className="mb-4 leading-7 text-pretty first:mt-0">{children}</div>
  ),
  strong: ({ children }: React.ComponentPropsWithoutRef<"strong">) => (
    <strong className="font-semibold text-foreground">{children}</strong>
  ),
  blockquote: ({ children }: React.ComponentPropsWithoutRef<"blockquote">) => (
    <blockquote className="border-l-4 border-primary pl-4 italic my-6 text-muted-foreground bg-muted/30 py-2 pr-4 rounded-r-sm first:mt-0">
      {children}
    </blockquote>
  ),
...
export function MarkdownRenderer({
  ...
  return (
    ...
    >
      <Streamdown plugins={{ code }} components={components}>
        {memoContent}
      </Streamdown>
    </div>
  );
